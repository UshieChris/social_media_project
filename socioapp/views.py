...
def index(request):
    ...
    return render(request, 'index.html', {'user_profile':user_profile, 'posts':feed_list, 'user_object':user_object, 'suggestions_username_profile_list':suggestions_username_profile_list[:4]})
...
def profile(request, pk):
    ...
    context = {'user_object':user_object, 'user_profile':user_profile, 'user_posts':user_posts, 'user_post_length':user_post_length, 'user_followers':user_followers, 'user_following':user_following, 'button_text': button_text}
...
def follow(request):
    if request.method == 'POST':
        ...
        if FollowersCount.objects.filter(follower = follower, user = user).first():
            ...
            new_followers = FollowersCount.objects.create(follower = follower, user = user)
            new_followers.save()
            return redirect('/profile/'+user)
    else:
        return redirect('/')
